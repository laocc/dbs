...
class Mysql
{
    ...
    public function table($tabName)
    {
        ...
        return (new Builder($this, $this->_CONF['table']))->table($tabName);
    }
    ...
    public function trans($trans_id = 1)
    {
        ...
        if ($trans_id === 0) {
            throw new \Exception("Trans Error: 多重事务ID须从1开始，不可以为0。");
        }
        ...
        return new Builder($this, $this->_CONF['table'], $trans_id);
    }
    ...
    private function connect($upData, $trans_id = 0)
    {
        ...
        //不是更新操作时，选择从库，需选择一个点
        if (!$upData) $host = $host[ip2long(getenv('REMOTE_ADDR')) % count($host)];
        ...
        if (!empty($option['param']) or $option['prepare']) {
            try {
                ...
                if ($stmt === false) {//预处理时就出错，一般是不应该的，有可能是字段名不对等等
                    ...
                    throw new \Exception('PDO_Error :  Prepare Update = ' . print_r($error, true));
                }
            ...
            try {
                ...
                $this->trans_back($CONN, $option['trans_id'], $error);//回滚事务
                if ($this->_try_error_throw) {
                    throw new \Exception('PDO_Error :  Execute Update = ' . print_r($error, true));
                }
                return self::_ERROR_RETURN;
            }
            ...
            try {
                ...
                $this->trans_back($CONN, $option['trans_id'], $error);//回滚事务
                if ($this->_try_error_throw) {
                    throw new \Exception('PDO_Error :  Exec Update = ' . print_r($error, true));
                }
                return self::_ERROR_RETURN;
            }
        }
        ...
        if (!empty($option['param']) or $option['prepare']) {
            try {
                ...
                if ($stmt === false) {
                    $error = $CONN->errorInfo()[2];
                    $this->trans_back($CONN, $option['trans_id'], $error);//回滚事务
                    throw new \Exception('PDO_Error :  Prepare Delete = ' . print_r($error, true));
                }
            } catch (\PDOException $PdoError) {//执行预处理，如果出错，很少见，还没遇到过
                $error = $PdoError->getMessage();
                $this->trans_back($CONN, $option['trans_id'], $error);//回滚事务
            ...
            try {
                ...
                $this->trans_back($CONN, $option['trans_id'], $error);//回滚事务
                if ($this->_try_error_throw) {
                    throw new \Exception('PDO_Error :  Execute Delete = ' . print_r($error, true));
                }
                return self::_ERROR_RETURN;
            }
            ...
            try {
                ...
                $this->trans_back($CONN, $option['trans_id'], $error);//回滚事务
                if ($this->_try_error_throw) {
                    throw new \Exception('PDO_Error :  Exec Delete = ' . print_r($error, true));
                }
                return self::_ERROR_RETURN;
            }
        }
        ...
        if (!empty($option['param']) or $option['prepare']) {
            ...
            try {
                ...
                if ($stmt === false) {
                    $error = $CONN->errorInfo()[2];
                    $this->trans_back($CONN, $option['trans_id'], $error);//回滚事务
                    throw new \Exception('PDO_Error :  Prepare Insert = ' . print_r($error, true));
                }
            } catch (\PDOException $PdoError) {//执行预处理，如果出错，很少见，还没遇到过
                $error = $PdoError->getMessage();
                $this->trans_back($CONN, $option['trans_id'], $error);//回滚事务
            ...
            if (!empty($option['param'])) {//有后续参数
                foreach ($option['param'] as &$row) {
                    try {
                        ...
                        $this->trans_back($CONN, $option['trans_id'], $error);//回滚事务
                        if ($this->_try_error_throw) {
                            throw new \Exception('PDO_Error :  Execute Insert = ' . print_r($error, true));
                        }
                        return self::_ERROR_RETURN;
                    }
                }
            ...
            try {
                ...
                $this->trans_back($CONN, $option['trans_id'], $error);//回滚事务
                if ($this->_try_error_throw) {
                    throw new \Exception('PDO_Error :  Exec Insert = ' . print_r($error, true));
                }
                return self::_ERROR_RETURN;
            }
        }
        ...
        if (!empty($option['param']) or $option['prepare']) {
            try {
                ...
                if ($stmt === false) {
                    ...
                    throw new \Exception('PDO_Error :  Prepare Select = ' . print_r($error, true));
                }
            } catch (\PDOException $PdoError) {//执行预处理，如果出错，很少见，还没遇到过
            ...
            try {
                ...
                $error = [$PdoError->getCode(), $PdoError->getMessage()];
                if ($this->_try_error_throw) {
                    throw new \Exception('PDO_Error :  Execute Select = ' . print_r($error, true));
                }
                return self::_ERROR_RETURN;
            }
        } else {
            try {
                ...
                $error = [$PdoError->getCode(), $PdoError->getMessage()];
                if ($this->_try_error_throw) {
                    throw new \Exception('PDO_Error :  Query Select = ' . print_r($error, true));
                }
                return self::_ERROR_RETURN;
            }
        }
    ...
    public function saveLog($sql, $title = null)
    {
        ...
        $text['url'] = getenv("REQUEST_URI");
        $text['ip'] = getenv('REMOTE_ADDR');
